...
// level type-checks against a grandchild route and then never renders it. The
// branch is required — `RouteProps` is a union, and an index route may not
// carry children, so passing both fails to compile.
...
export default function App() {
  return (
    <Routes>
      {appRoutes.map((route, i) => renderRoute(route, route.path ?? `layout-${i}`))}
    </Routes>
  );
}
